...
# ============================================================================
# THRESHOLD FUNCTIONS (SAME AS main.py)
# ============================================================================

def find_optimal_thresholds(y_true, y_scores):
    """Find optimal thresholds per class using ROC curve (same as main.py)"""
    thresholds = []
    for i in range(y_true.shape[1]):
...
def apply_thresholds(y_scores, thresholds):
    """Apply class-wise thresholds (same as main.py)"""
    y_pred = (y_scores > thresholds).astype(int)
...
def train_model(config, metadata, device):
    ...
    for epoch in range(EPOCHS):
        ...
        # Find optimal thresholds and compute metrics (SAME AS main.py)
    ...
    # Test with best model and optimal thresholds (SAME AS main.py)
    print(f"\nTesting {config['name']}...")
    checkpoint = torch.load(os.path.join(PROCESSED_PATH, f"best_{config['name']}.pth"))
    model.load_state_dict(checkpoint['model_state_dict'])
    ...
    # Apply optimal thresholds found during validation (SAME AS main.py)
